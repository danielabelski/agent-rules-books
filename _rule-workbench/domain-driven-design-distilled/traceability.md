...
- Dropped from `nano.md` as lower-yield: some collaboration and review phrasing that supports DDD but does not itself change model shape.
...
- `M1` Start with subdomains and bounded contexts; do not apply DDD uniformly everywhere. Source: `Strategic Rules` (38-69), `Practicality Rules` (197-210).
- `M2` Use ubiquitous language consistently across code and conversation. Source: `Ubiquitous Language Rules` (70-87).
- `M3` Use tactical patterns only when they express domain meaning: entities, value objects, aggregates, repositories, domain events, domain services. Source: `Tactical Pattern Rules` (88-139).
...
- `M5` Keep application services thin; the domain model owns business rules, infrastructure stays outside. Source: `Application Service Rules` (155-168), `Architecture and Infrastructure Rules` (169-182).
- `M6` Keep the approach practical: use richer modeling where complexity earns it and simpler styles where it does not. Source: `Practicality Rules` (197-210).

## Nano mapping

- `N1` Start with subdomains and bounded contexts; do not apply DDD everywhere. Source: `Strategic Rules` (38-69), `Practicality Rules` (197-210).
